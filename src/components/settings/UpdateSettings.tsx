    ...
    <div className="flex flex-col gap-5">
      <div className="flex items-center justify-between rounded-xl bg-black/[0.03] dark:bg-white/[0.04] p-3">
        <div>
          ...
          <p className="text-xs text-neutral-500 dark:text-neutral-400">
            {lastCheckedAt
              ? t("updates.lastChecked", { time: formatDistanceToNowStrict(lastCheckedAt, { addSuffix: true }) })
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-accent/20 bg-accent/5 p-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-neutral-800 dark:text-neutral-100">
              {t("updates.available", { version: info.version })}
            </p>
          </div>
          {info.notes && (
      ...
      {status === "downloading" && (
        <div className="flex flex-col gap-2 rounded-xl border border-accent/20 bg-accent/5 p-3">
          <p className="text-sm font-medium text-neutral-800 dark:text-neutral-100">{t("updates.downloading")}</p>
      ...
      {status === "ready" && (
        <div className="flex items-center justify-between rounded-xl border border-success/20 bg-success/5 p-3">
          <p className="text-sm font-medium text-neutral-800 dark:text-neutral-100">{t("updates.ready")}</p>
      ...
      {status === "error" && (
        <div className="rounded-xl border border-danger/20 bg-danger/5 p-3">
          <p className="text-sm font-medium text-danger">{t("updates.failed")}</p>
          <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{error}</p>
        </div>
      )}

      <p className="text-[11px] text-neutral-400">{t("updates.footer")}</p>
    </div>
  );
}
