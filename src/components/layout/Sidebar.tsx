    ...
    <aside className="glass-panel flex w-60 flex-shrink-0 flex-col rounded-none border-y-0 border-l-0 p-3">
      <div className="relative mb-3">
        <button
          ...
          className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left hover:bg-black/5 dark:hover:bg-white/10"
        >
          <span
        ...
        {switcherOpen && (
          <div className="glass-panel-elevated absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-xl p-1">
            {accounts.map((account) => (
              <button
                ...
                className={clsx(
                  "flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm hover:bg-black/5 dark:hover:bg-white/10",
                  account.id === activeAccountId && "bg-accent/10 text-accent",
                )}
              >
                <span className="h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: account.color }} />
      ...
      <button
        onClick={() => openSettings("signatures")}
